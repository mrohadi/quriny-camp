  ...
  async.waterfall([
    ...
    function (token, user, done) {
      var smtpTransport = nodemailer.createTransport({
        service: 'Gmail',
        auth: {
          user: 'muhammadrohadi03@gmail.com',
          pass: process.env.GMAILPW
        }
      });
      var mailOptions = {
        to: user.email,
        from: 'muhamadrohadi03@gmail.com',
...
router.post('/reset/:token', function (req, res) {
  async.waterfall([
    ...
    function (user, done) {
      ...
      var mailOptions = {
        to: user.email,
        from: 'muhammadrohadi03@gmail.com',
...
router.get('/profile/:id', (req, res) => {
  User.findById(req.params.id, (err, user) => {
    ...
    Campground.find().where('author.id').equals(user._id).exec((err, campgrounds) => {
      ...
      res.render('users/infoUser', { user: user, campgrounds: campgrounds });
    })
  })
})
